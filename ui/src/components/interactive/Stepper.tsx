...
export const Stepper: React.FC<Props> = ({ item, handleQuantityChange }) => {
  return (
    <div className="flex gap-4 items-center">
      <Button variant="secondary" onClick={() => handleQuantityChange(item.itemId, '-')} title="Remove One">
        <Icon path={mdiMinus} size={0.55} />
      </Button>
      <p title={`Current quantity: ${item.quantity}`}>{item.quantity}</p>
      <Button variant="secondary" onClick={() => handleQuantityChange(item.itemId, '+')} title="Add One">
